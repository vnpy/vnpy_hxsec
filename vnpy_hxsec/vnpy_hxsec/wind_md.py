...
from vnpy_hxsec.event import EVENT_WIND
...
class WindMdApi:
    ...
    wsq_param_map["rt_high_limit"] = "limit_up"
    wsq_param_map["rt_low_limit"] = "limit_down"
    ...
    def connect(self):
        ...
        if not data.ErrorCode:
            ...
            self.gateway.on_event(EVENT_WIND, True)
        else:
            self.gateway.write_log("Wind行情接口连接失败")
            self.gateway.on_event(EVENT_WIND, False)
            return
